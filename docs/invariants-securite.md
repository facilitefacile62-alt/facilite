...
**Ce qu'il protège** : Trois fois la même classe de bug a cassé le projet : le lien Admin conditionné à `profiles.role='admin'`, les policies Storage conditionnées à `role='recruteur'`, et le lien Recruteur conditionné à `profileRole==='recruteur'`. Depuis la migration RBAC, ces littéraux de rôles (`'recruteur'`, `'candidat'`, `'agent'`, `'entreprise'`) n'existent plus dans `user_roles`. Ce test scanne tous les fichiers `.js/.jsx` de `src/` pour identifier les littéraux de rôles obsolètes utilisés dans des CONDITIONS de gate, sous deux formes : comparaison directe (`===`/`!==`/`==`/`!=`) et test d'appartenance (`.includes("recruteur")`) — excluant les usages décoratifs légitimes (ex: `RoleBadge role="candidat"`).

**Élargi le 2026-08-06** (Partie 9 du chantier, `docs/incident-2026-08-06.md`) : `'entreprise'` ajouté à la liste (4e littéral du modèle pré-RBAC, cf. Invariant 7 volet 2 — absent d'ici jusque-là sans raison) et la forme `.includes(...)` ajoutée aux motifs détectés — aucune violation active des deux, mais la même classe de bug sous une syntaxe différente serait passée inaperçue. Balayage complémentaire effectué à cette occasion : routes/middleware (`src/middleware.js`, `src/lib/roles.js`) déjà propres, lisent `user_roles.role` directement, aucun littéral obsolète.
